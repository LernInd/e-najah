...
type PerizinanView = "dashboard" | "pengajuan" | "status" | "surat" | "kembali" | "buat_pengajuan_form";
  ...
  return (
    <nav className="dashboard-navbar">
      ...
      <div className="navbar-links">
        ...
        <button className={navActiveView === "kembali" ? "active" : ""} onClick={() => onNavigate("kembali")}> Kembali </button>
      </div>
...
const SuratView: React.FC = () => { 
  ...
  const handlePrint = useReactToPrint({ contentRef: componentToPrintRef, onAfterPrint: () => setDataToPrint(null), });
  useEffect(() => { const fetchApproved = async () => { setIsLoading(true); setError(""); try { const token = getToken(); const response = await fetch("/api/admin/perizinan/all", { headers: { Authorization: `Bearer ${token}` }, }); const data = await response.json(); if (!response.ok) { throw new Error(data.error || "Gagal mengambil data"); } setApprovedList(data.results.filter((p: SemuaPengajuanData) => p.keputusan === 'disetujui')); } catch (err: any) { setError(err.message); } finally { setIsLoading(false); } }; fetchApproved(); }, []);
  const triggerPrint = (data: SemuaPengajuanData) => { if (!data.ID_Perizinan) { alert("Error: Data perizinan tidak lengkap, ID Perizinan tidak ditemukan."); return; } const suratData: SuratIzinData = { ID_Perizinan: data.ID_Perizinan, nama_santri: data.nama_santri, alamat: data.alamat, foto: data.foto, Tanggal_Kembali: data.Tanggal_Kembali, disetujui_oleh: data.disetujui_oleh, }; setDataToPrint(suratData); };
  ...
  return (
    <div className="placeholder-page">
      ...
      {!isLoading && (
        <div className="search-results-container">
          <table className="results-table">
            <thead> <tr> <th>No. Surat</th> <th>Nama Santri</th> <th>Pengajuan</th> <th>Disetujui Oleh</th> <th>Aksi</th> </tr> </thead>
            <tbody>
              {approvedList.length === 0 && ( <tr> <td colSpan={5} style={{ textAlign: 'center', color: '#888' }}> Belum ada pengajuan yang disetujui. </td> </tr> )}
              {approvedList.map((p) => (
                <tr key={p.ID_Pengajuan}>
                  ...
                  <td> <button className="print-button" onClick={() => triggerPrint(p)}> Cetak </button> </td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {dataToPrint && ( <SuratIzinA5 ref={componentToPrintRef} data={dataToPrint} /> )}
    </div>
  );
};
...
const StatusView: React.FC = () => {
  ...
  useEffect(() => { fetchAllPengajuan(); }, []);
  const fetchAllPengajuan = async () => { setIsLoading(true); setError(""); try { const token = getToken(); const response = await fetch("/api/admin/perizinan/all", { headers: { Authorization: `Bearer ${token}` }, }); const data = await response.json(); if (!response.ok) { throw new Error(data.error || "Gagal mengambil data"); } setAllPengajuan(data.results); } catch (err: any) { setError(err.message); } finally { setIsLoading(false); } };
  const filteredList = useMemo(() => { return allPengajuan.filter(p => p.keputusan === activeTab); }, [allPengajuan, activeTab]);
  return (
    <div className="placeholder-page">
        ...
        <tbody>
          {list.map((p) => (
            <tr key={p.ID_Pengajuan}>
              ...
              <td className={p.Keterlambatan_Jam && p.Keterlambatan_Jam > 0 ? 'text-late' : ''}>
...
const KembaliView: React.FC = () => {
  ...
  const formatTanggalKembali = (tanggal: string) => {
    ...
    const sisaHari = Math.ceil((tglKembali.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
    const tglFormatted = tglKembali.toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' });
            ...
            <table className="results-table">
              ...
              <tbody>
                {izinAktifList.length === 0 && (
                  <tr> <td colSpan={4} style={{ textAlign: 'center', color: '#888' }}> Tidak ada santri yang sedang izin. </td> </tr>
                )}
                {izinAktifList.map((izin) => (
                  <tr key={izin.ID_Perizinan}>
                    <td>{izin.nama_santri}</td>
                    <td>{izin.nama_pengajuan}</td>
                    <td className={new Date(izin.Tanggal_Kembali) < new Date() ? 'text-late' : ''}>
...
const KembaliModal: React.FC<KembaliModalProps> = ({ izin, onClose, onSubmitSuccess }) => {
  const [statusKembali, setStatusKembali] = useState<StatusKembali>("Tepat Waktu");
  const [keterlambatanHari, setKeterlambatanHari] = useState("0");
  const [keterlambatanJam, setKeterlambatanJam] = useState("0"); // <-- State BARU
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState("");
    ...
    try {
      ...
      const response = await fetch("/api/admin/perizinan/tandai-kembali", {
        ...
        body: JSON.stringify({ 
          ...
          keterlambatanJam: parseInt(keterlambatanJam) || 0 // <-- Kirim data jam
        }),
      });
      const data = await response.json();
      if (!response.ok) {
      ...
      if (data.status === 'Terlambat') {
        message += ` Status: Terlambat (${formatKeterlambatan(data.keterlambatanJam)})`;
      } else {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="detail-info-grid simple-grid">
          <div className="detail-item"> <label>Nama Santri</label> <p>{izin.nama_santri}</p> </div>
          <div className="detail-item"> <label>Izin</label> <p>{izin.nama_pengajuan}</p> </div>
        </div>
          ...
          <div className="form-grid">
            <div className="form-group form-span-2">
              ...
              <select 
                id="status_kembali" 
                value={statusKembali} 
                onChange={(e) => setStatusKembali(e.target.value as StatusKembali)}
              >
            ...
            {/* Input Keterlambatan (DIPERBARUI) */}
...
const DashboardAdminPerizinan: React.FC<DashboardAdminPerizinanProps> = ({
  ...
  const renderView = () => {
    switch (view) {
      ...
      case "kembali": return <KembaliView />; // <-- VIEW BARU
  ...
  return (
    <div className="dashboard-layout">
      <PerizinanNavbar 
        ...
        activeView={view} 
        onNavigate={(v) => { setView(v); setSelectedSantri(null); }} 
      />
